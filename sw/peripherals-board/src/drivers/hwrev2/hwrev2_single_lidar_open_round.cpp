...
VehicleCommand hw_rev_2_SingleLidarOpenRound::drive(VehicleData vehicleData){
  ...
  if (turning == true){
    ...
    if (abs(difference) <= 6.5f){   // Return to straight after turning for ~83.5°
        speed = 300;
        turning = false;
        encoderValue = 0;
        distance = 0;
    ...
    else if (abs(difference) > 10 && abs(difference) < 75){ // 75° to 10°
    ...
    else if(abs(difference) > 75) pos = 90 + turnDir * 70; //90° to 75°
  ...
  if(!turning && front_lidarDist <= threshold and abs(error) < 15){

    if(turns == 0){

      speed = 150;
      if (left_lidarDist + right_lidarDist > 120){
        speed = 300;
        turning = true;
